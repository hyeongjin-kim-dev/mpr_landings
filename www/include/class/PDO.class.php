 ...
 */
require(trim($_SERVER['DOCUMENT_ROOT']) . '/include/class/PDO.Log.class.php');
require(trim($_SERVER['DOCUMENT_ROOT']) . '/include/class/PDO.Iterator.class.php');
...
class DB
{
	 ...
	 * 디비 자체 함수로 암호화 한다.
	 */
	public function hexAesEncrypt($keyWord) {
		$_Key = _KEY;
		$strSQL = " select hex(aes_encrypt('{$keyWord}','{$_Key}')) ";
		return $this->single($strSQL);
	}
	/**
	 * 디비 자체 함수로 복호화 한다.
	 */
	public function hexAesDecrypt($keyWord) {
		$_Key = _KEY;
		$strSQL = " select aes_decrypt(unhex('{$keyWord}'), '{$_Key}') ";
		return $this->single($strSQL);
	}
}
