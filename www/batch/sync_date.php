...
function db_landings($rs){
    ...
    if(count($rs) > 0 ){
        ...
        for($i=0; $i < count($rs) ; $i++){
            ...
            $from = strtotime($rs[$i]['expose_from']);
            $to = strtotime($rs[$i]['expose_to']);
            ...
            if($cnt[0] > 0){
                ...
                if($rs[$i]['expose_always'] == 'Y'){
                    ...
                    if($from > $str_now){
                        $ev_stat = "W";
                    }else if($from <= $str_now && $str_now <= $to){
                        $ev_stat = "Y";
                    }else{
                        $ev_stat = "N";
                    }
                ...
                if(mysqli_query($connect_ld, $sql)){
                    echo "updated - code : ".$rs[$i]['event_cd']." from : ".$rs[$i]['expose_from']." to : ".$rs[$i]['expose_to']." always : ".$rs[$i]['expose_always']." stat : ".$ev_stat."\n";                    
                }else{
                    echo "error : ".$sql."\n".mysqli_error($conn)."\n";
                }        
            }    
